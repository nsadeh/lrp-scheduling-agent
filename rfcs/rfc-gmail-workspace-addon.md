...
- **No push updates.** The sidebar only refreshes on user actions (button clicks, opening a new message). No WebSocket/SSE. Status data is recalculated server-side on each interaction.
  - **Stale-homepage handling (resolved).** Previously, after a coordinator acted on a suggestion inside a contextual (thread) card, Gmail would keep serving its cached homepage card on next view — showing the just-resolved suggestion. This is **fixed and confirmed in production**: mutating `/addon/action` handlers return the `renderActions` envelope with a **top-level `stateChanged: true`** sibling, which makes Gmail clear the cached homepage so the next view re-renders fresh. Two earlier `stateChanged` placements were reverted (`6d878dc`: bare-`action` sibling; `0b995bd`: nested inside `renderActions.action`) — both rejected by Gmail's validator; the current top-level-sibling placement is the documented one and is verified working live, including on the homepage surface. Independently, **all mutating handlers are idempotent** — they no-op (returning the scoped post-action view) if the suggestion/draft is no longer `pending`/`generated`, so even a stale double-click can never re-send an email or re-advance a stage. The homepage `↻ Refresh` button (`OpenLink` overlay + `onClose:RELOAD`) remains as a manual remedy but is no longer required for this case.
