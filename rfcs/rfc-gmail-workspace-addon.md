...
- **No push updates.** The sidebar only refreshes on user actions (button clicks, opening a new message). No WebSocket/SSE. Status data is recalculated server-side on each interaction.
  - **Stale-homepage caveat & current handling.** After a coordinator acts on a suggestion inside a contextual (thread) card, Gmail may keep serving its cached homepage card on next view — showing the just-resolved suggestion. Two mitigations are in place: (1) **all mutating `/addon/action` handlers are idempotent** — they no-op (returning the scoped post-action view) if the suggestion/draft is no longer `pending`/`generated`, so a stale double-click can never re-send an email or re-advance a stage; (2) mutating handlers return the `renderActions` envelope with a **top-level `stateChanged: true`** sibling, which per Google's docs lets Gmail clear cached state. Two earlier `stateChanged` placements were reverted (`6d878dc`: bare-`action` sibling; `0b995bd`: nested inside `renderActions.action`) — both rejected by Gmail's validator. The current top-level-sibling placement is the documented one; whether Gmail honors it for the *homepage* surface specifically is verified empirically (manual test) — and the idempotency guard means the bug is harmless even if it doesn't. The homepage `↻ Refresh` button (`OpenLink` overlay + `onClose:RELOAD`) remains the reliable manual remedy.
